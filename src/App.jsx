...
const Icon = {
  ...
  category:  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 6h16M4 12h10M4 18h7"/></svg>,
  item:      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>,
  logout:    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2h6a2 2 0 012 2v1"/></svg>,
  tag:       <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M7 7h.01M7 3h5a2 2 0 011.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A2 2 0 013 12V7a4 4 0 014-4z"/></svg>,
  menuitem:  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="9"/><path d="M12 8v4l3 3"/></svg>,
  ham:       <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 6h16M4 12h16M4 18h16"/></svg>,
};
  ...
  const DashboardView = () => (
    <div>
      ...
      <div className="welcome-banner">
        ...
        <p>Here's what's happening with <strong>{user.businessName}</strong> today.</p>
      </div>
      ...
      <div className="quick-actions">
        <button className="qa-btn qa-blue"   onClick={() => setActiveTab('categories')}>＋ Add Category</button>
        <button className="qa-btn qa-green"  onClick={() => setActiveTab('items')}>＋ Add Menu Item</button>
        <button className="qa-btn qa-purple" onClick={() => setActiveTab('categories')}>📂 Manage Categories</button>
        <button className="qa-btn qa-orange" onClick={() => setActiveTab('items')}>📋 Manage Items</button>
      </div>
      ...
      <div className="overview-grid">
            ...
            : <ul className="ov-list">
                {categories.slice(-5).reverse().map(c => (
                  <li key={c.id} className="ov-item">
                    <span className="ov-dot" style={{ background: '#6366f1' }} />
                    {c.name}
                  </li>
                ))}
              </ul>
        ...
        <div className="overview-card">
          ...
          {menuItems.length === 0
            ? <p className="ov-empty">No menu items yet.</p>
            : <ul className="ov-list">
      ...
      <div className="main-area">
        ...
        <header className="topbar">
          ...
          <div className="topbar-right">
            <span className="topbar-badge">● Online</span>
        ...
        <main className="page-content">
          ...
          {activeTab === 'items'      && <MenuItemManager categories={categories} items={menuItems} setItems={setMenuItems} />}
        </main>
      </div>
    </div>
  );
